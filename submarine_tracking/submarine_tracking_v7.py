...
# Add a column for the ocean current velocity
df['ocean_current_velocity'] = pd.read_csv("ocean_current_velocity.csv")

# Add a column for the tidal height
df['tidal_height'] = pd.read_csv("tidal_height.csv")
...
# Calculate the ocean current force
ocean_current_force = water_density * df['ocean_current_velocity']**2 * submarine_cross_sectional_area

# Calculate the tidal force
tidal_force = (df['tidal_height'].diff() / df['timestamp'].diff()) * submarine_mass * 9.8

# Calculate the net force on the submarine
net_force = submarine_mass * df['acceleration'] + drag_force - buoyancy_force + ocean_current_force + tidal_force
...
plt.legend()
plt.xlabel('Timestamp')
plt.ylabel('Speed/Velocity (m/s)')

plt.show()

#Plot the water temperature, pressure, and salinity
...
plt.subplot(133)
plt.plot(df['timestamp'], df['water_salinity'], label='Water Salinity')
plt.legend()
plt.xlabel('Timestamp')
...
#Plot the ocean current velocity and tidal height

plt.figure(figsize=(15,5))
plt.subplot(121)
plt.plot(df['timestamp'], df['ocean_current_velocity'], label='Ocean Current Velocity')
plt.legend()
plt.xlabel('Timestamp')
plt.ylabel('Velocity (m/s)')

plt.subplot(122)
plt.plot(df['timestamp'], df['tidal_height'], label='Tidal Height')
plt.legend()
plt.xlabel('Timestamp')
plt.ylabel('Height (m)')
